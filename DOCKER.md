...
Новые XA добавляются в очередь сразу после импорта. Уже находящиеся в Orthanc
исследования готовятся по первому запросу клиента; массовый прогрев при каждом
старте отключён. Первая XA-серия публикуется сразу после подготовки, остальные
серии продолжают готовиться в фоне. Прогресс виден в логах backend по строкам
`XA cine cache`. ZIP кадров по умолчанию не создается, поскольку мобильный
клиент сохраняет MP4, а точные JPEG получает только для выбранной серии.

Протоколы прошлых недель остаются в PostgreSQL для годовой статистики и
истории, а недельный список ограничивает API. Фоновое soft-delete retention
отключено по умолчанию (`STUDY_RETENTION_ENABLED=false`).
...
## Пользователи и персональный диск

Миграции создают встроенные учётные записи Viewer. Пользователи могут менять
собственный логин и пароль. Метаданные файлов и лимит 1 ГБ хранятся в
PostgreSQL. При настроенных `YANDEX_*` сами файлы сохраняются в приватном
Object Storage под префиксом `viewer-drive/<user-id>/`.

Переменная `DRIVE_YANDEX_BUCKET` позволяет выделить отдельный bucket. При
пустом значении используется `YANDEX_BUCKET`. Пользовательский диск работает
только через приватный Object Storage; локальная постоянная копия файлов не
создаётся. Если облачные реквизиты не настроены, файловые операции возвращают
ошибку доступности хранилища.
