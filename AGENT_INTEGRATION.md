...
полный протокол в `/studies`. `sync_studies` немедленно проверяет все каталоги
протоколов и отправляет новые записи в `/studies`. План операций хранится только
в backend и изменяется web/mobile через `/operation-plan`; агент не читает
локальный план. Команды `send_*_to_pacs` требуют `study_uid` и принудительно
